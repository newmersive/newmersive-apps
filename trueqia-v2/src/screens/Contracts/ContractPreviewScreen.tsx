...
export default function ContractPreviewScreen({ route }: any) {
  ...
  const offerTitle = useMemo(
    () => trade?.title || offer?.title || "Oferta",
  ...
  const payload = useMemo(
    ...
    [offer?.description, offer?.id, offerTitle, providerName, requesterName, tokens, trade?.id, trade?.offerId, trade?.ownerId, trade?.title, user?.id, notes]
  );
