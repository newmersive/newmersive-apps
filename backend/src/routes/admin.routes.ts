...
router.get("/dashboard", authRequired, adminOnly, (req: Request, res: Response): void => {
  try {
...
router.get("/summary", authRequired, adminOnly, (req: Request, res: Response): void => {
  try {
    const db = getDatabase();
...
router.get("/users", authRequired, adminOnly, async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get("/leads", authRequired, adminOnly, (req: Request, res: Response): void => {
  try {
...
router.get("/ai/activity", authRequired, adminOnly, (req: Request, res: Response): void => {
  try {
    const events = [
      {
        id: "demo-1",
